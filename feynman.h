...
static inline uint32_t feynman_nlog_5_27(
    ...
    /* Arg is now [0,5,1[ in 0.32 fixed point, or
                  [1,2[   in 1.31 fixed point.
       Convert it to 2.30 fixed point to fit core routine range. */
    arg >>= 1;
    uint32_t fine = feynman_log_1_2(table, precision, arg) >> point;

    /* Return negative log, compensating for the extra shift. */
    return coarse - fine + one;
}

/* We throw away 5 bits, so we can truncate the table after 26
 * entries. */
#define FEYNMAN_ITERATIONS 26
