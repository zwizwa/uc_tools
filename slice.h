...
/* Slices with slice operations.  The slice ops will "consume" the
   slices, i.e. buf and len will be updated.  That seems to be most
   convenient in actual use. */
...
#define DEF_SLICE_ALL(_type) \
    ...
    void put_slice_##_type(slice_##_type *s, _type d) { *s->buf++ = d; s->len--; } \
