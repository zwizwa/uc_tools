#!/usr/bin/env lua
-- Test for backend_erlang Erlang code printer pass.
...
local backend_erlang = require('lure.backend_erlang')
require('lure.log_se')
local ins = table.insert

-- The Erlang backend input requires IR, so we generate that from Scheme.
local c_new =
...
local function run()
   local str = asset['test_backend_erlang.scm']
   assert(str)
