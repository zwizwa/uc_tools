#ifndef TRACE_IMPL_BIN_H
#define TRACE_IMPL_BIN_H

/* EXPERIMENTAL */

/* Derived from trace_impl_default.h
   Tags are single bytes which need to be defined as macros trace_tag_<tag>
   Types are functions trace_write_bin_<type>. */
...
#define DECL_TRACE_TAG_MEMBER(name)  uint8_t name;
...
#define DEF_TRACE_TAG(name) .name = trace_tag_##name,
#define DEF_TRACE_TAGS_STRUCT(name) const struct trace_tags name = { FOR_TRACE_TAGS(DEF_TRACE_TAG) }

#define DEF_TRACE_TYPE(name) .name = trace_bin_##name,
